...
$difficulty = strip_tags($data['difficulty']);
$platform = isset($data['platform']) ? strip_tags($data['platform']) : 'Unknown';

try {
    // Preparar INSERT SQL
    $stmt = $pdo->prepare("INSERT INTO snake_analytics (player_name, score, duration_seconds, moves_count, max_speed_ms, death_type, difficulty, platform) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");

    // Ejecutar
    $stmt->execute([$player_name, $score, $duration, $moves, $max_speed, $death_type, $difficulty, $platform]);
